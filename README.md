...
Every row action is a visible button on the row — no overflow menu. Inbox: **Today / SD / ✕** (SD is "Someday"; the full word next to "Today" reads as one run of text in the squeezed tap boxes — desktop spells it out), Today: **checkbox / ↑ / Inbox**, Someday: **Inbox / ✕**, Recycle: **Restore**. Rows also swipe as an accelerator: **right is constructive, left removes**, mirroring the buttons. Anything destructive raises an Undo toast, and a test asserts every swipe action exists as a button structurally.
