...
- **Mobile-first shell.** Touch is the base layout; `@media (min-width: 768px)`
  ...
  Four tabs over a fixed dock, hash routing, popup-card dialogs, every row
  action an always-visible button (no overflow menu; swipe kept as an
  accelerator with Undo), in-app toast and confirm replacing
  `alert()`/`confirm()`.
